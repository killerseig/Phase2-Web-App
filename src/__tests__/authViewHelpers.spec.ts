...
describe('auth view helpers', () => {
  ...
  it('reads auth email query prefill from scalar or repeated query values', () => {
    ...
    expect(getAuthEmailQueryPrefill({ email: ['cj@example.com', 'other@example.com'] })).toBe('cj@example.com')
  ...
  it('redirects to the workspace only when the current auth state allows access', async () => {
    ...
    await expect(redirectToWorkspaceIfAllowed({
  ...
  it('validates password setup fields in the same order as the route form', () => {
    ...
    expect(getSetPasswordValidationMessage('short', 'short')).toBe('Password must be at least 6 characters.')
    expect(getSetPasswordValidationMessage('password1', 'password2')).toBe('Passwords do not match.')
