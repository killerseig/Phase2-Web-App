import type { DiffField, WorkbookOffField } from '@/types/timecards'
import { makeDaysArray, type TimecardJobUi, type TimecardModel } from '@/utils/timecardUtils'
...
export function useTimecardJobEditing(options: UseTimecardJobEditingOptions) {
  ...
  const updateOffValue = (timecard: TimecardModel, index: number, field: WorkbookOffField, rawValue: number) => {
    const job = timecard.jobs?.[index]
    if (!job) return
    job[field] = toNonNegative(rawValue)
    recalcTotals(timecard)
    autoSave(timecard)
  }
  ...
  const handleUnitCostInput = (
    ...
    const day = job ? getJobDay(job, dayIndex) : null
    if (!day) return
    day.unitCostOverride = rawValue == null ? null : toNonNegative(rawValue)
    recalcTotals(timecard)
    autoSave(timecard)
  }
