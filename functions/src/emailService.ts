...
import type { DailyLogInlinePhotoPreview, DailyLogPhotoSectionKey } from './dailyLogEmailPhotos'
import { EmailDeliveryError } from './emailDeliveryErrors'
...
async function getGraphAuthToken(): Promise<string> {
  ...
  try {
    ...
    const failure = new EmailDeliveryError(`Failed to get Graph API token: ${error.message}`, {
...
interface DailyLogEmailAttachment {
  name: string
  description: string
...
function normalizeDailyLogPhotoSection(value: unknown): DailyLogPhotoSectionKey {
  if (value === 'ptp' || value === 'qc') return value
  // "other", missing, and unknown legacy types were historically shown with Photos.
  return 'photo'
}

function normalizeDailyLogEmailAttachments(value: unknown): DailyLogEmailAttachment[] {
...
function buildDailyLogGalleryTarget(
  dailyLogUrl: string,
  section: DailyLogPhotoSectionKey,
  position?: number,
): string {
...
function renderDailyLogPhotoSection(
  title: string,
  section: DailyLogPhotoSectionKey,
  allAttachments: DailyLogEmailAttachment[],
  dailyLogUrl: string,
  inlinePreviews: DailyLogInlinePhotoPreview[],
): string {
  ...
  const previewCells = previewAttachments.map((attachment, index) => {
    ...
    const inlinePreview = inlinePreviews.find(
      (preview) => preview.section === section && preview.position === index + 1,
    )
  ...
  for (let index = 0; index < previewCells.length; index += 2) {
    previewRows.push(
      `<tr>${previewCells[index]}${previewCells[index + 1] || '<td width="50%" style="width: 50%; padding: 0; border: 0;"></td>'}</tr>`,
    )
  }

  const sectionUrl = buildDailyLogGalleryTarget(dailyLogUrl, section)
  const overflowButton =
    attachments.length > DAILY_LOG_EMAIL_PHOTO_PREVIEW_LIMIT && sectionUrl
...
export function buildDailyLogEmail(
  ...
  const photosSection = renderDailyLogPhotoSection(
    'Photos',
    'photo',
    emailAttachments,
    dailyLogUrl,
  ...
  const ptpPhotosSection = renderDailyLogPhotoSection(
    'PTP Photos',
    'ptp',
    emailAttachments,
    dailyLogUrl,
  ...
  const qcPhotosSection = renderDailyLogPhotoSection(
    'QC Photos',
    'qc',
    emailAttachments,
    dailyLogUrl,
...
function getNextThursdayDateValue(reference: any): string {
...
function getShopOrderRequestedDeliveryDateValue(order: any): string {
  const explicitDeliveryDate = String(order?.deliveryDate || '').trim()
  if (explicitDeliveryDate) return explicitDeliveryDate

  return getNextThursdayDateValue(order?.orderDate || order?.createdAt || order?.updatedAt)
}

function normalizeShopOrderNumber(value: any): string {
...
function getShopOrderItemCostCode(
  ...
  const catalogCostCode = catalogItemId
    ...
    : ''
  if (catalogCostCode) return catalogCostCode
...
function normalizeShopOrderQuantity(value: any): number {
  return Math.max(0, Math.floor(Number(value) || 0))
}

function getShopOrderItemReceivedQuantity(item: any): number {
  return Math.min(
    normalizeShopOrderQuantity(item?.quantity),
    normalizeShopOrderQuantity(item?.receivedQuantity),
  )
}

function getShopOrderItemBackorderedQuantity(item: any): number {
  ...
  return Math.min(
    ...
    normalizeShopOrderQuantity(item?.backorderedQuantity),
  )
}

function getShopOrderItemPendingQuantity(item: any): number {
  const quantity = normalizeShopOrderQuantity(item?.quantity)
  return Math.max(
    0,
    quantity - getShopOrderItemReceivedQuantity(item) - getShopOrderItemBackorderedQuantity(item),
  )
}
...
function getShopOrderStatusLabel(status: any): string {
  ...
  if (normalized === 'received' || normalized === 'receive') return 'Received'
  return 'Draft'
}
...
function renderEmailText(value: any, fallback = 'N/A'): string {
  const text = String(value ?? '').trim()
  return escapeHtml(text || fallback)
}

function renderOptionalEmailText(value: any): string {
  const text = String(value ?? '').trim()
  return text ? escapeHtml(text) : '&nbsp;'
}

function normalizeRootFolderLabel(value: string): string {
  return value
    .toLowerCase()
    .replace(/[\u2019']/g, '')
    .replace(/[^a-z0-9]+/g, ' ')
...
function getShopOrderDescriptionSegments(description: any): string[] {
...
function getJobDisplayLabel(order: any): string {
  const jobCode = String(order?.jobCode || '').trim()
  const jobName = String(order?.jobName || '').trim()
  if (jobCode && jobName) return `${jobName} - ${jobCode}`
...
function renderPrintedOrderPlainField(value: string, align: 'left' | 'right' = 'left') {
  return `
    <div style="font-size: 14px; color: #202020; text-align: ${align};">
...
export async function sendEmail(options: SendEmailOptions): Promise<void> {
  ...
  if (invalidRecipientCount > 0) {
    throw new EmailDeliveryError(
      `${invalidRecipientCount} invalid email ${invalidRecipientCount === 1 ? 'address was' : 'addresses were'} provided.`,
      { retryable: false },
    )
  }
  ...
  try {
    ...
    const failure =
      error instanceof EmailDeliveryError
        ? error
        : new EmailDeliveryError(`Failed to send email: ${error.message}`, {
            httpStatus: error.response?.status,
          })
