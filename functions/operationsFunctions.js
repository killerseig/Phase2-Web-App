...
async function ensureForemanWorkspaceTimecards(jobId, weekEndingDate) {
    const rosterSnap = await runtime_1.db.collection(constants_1.COLLECTIONS.JOBS).doc(jobId).collection('roster').get();
    const rosterDocs = rosterSnap.docs
        .map((docSnap) => ({ id: docSnap.id, data: docSnap.data() }))
        .filter((entry) => entry.data?.active !== false);
    const timecardsSnap = await runtime_1.db
    ...
    for (const rosterEntry of rosterDocs) {
        ...
        if (!rosterEmployeeId || existingByRosterId.has(rosterEmployeeId))
            continue;
        await runtime_1.db.collection(constants_1.COLLECTIONS.JOBS).doc(jobId).collection('timecards').add({
            jobId,
            weekStartDate,
            weekEndingDate,
            status: 'draft',
    ...
    const refreshedTimecardsSnap = await runtime_1.db
        .collection(constants_1.COLLECTIONS.JOBS)
        .doc(jobId)
        .collection('timecards')
        .where('weekEndingDate', '==', weekEndingDate)
 ...
 */
exports.sendTimecardEmail = (0, https_1.onCall)({ secrets: (0, functionConfig_1.getGraphEmailSecrets)() }, async (request) => {
    if (!request.auth) {
        throw new Error(constants_1.ERROR_MESSAGES.NOT_SIGNED_IN);
    }
    const callerUid = request.auth.uid;
    ...
    try {
        ...
        if (timecards.length === 0) {
            const availableSnap = await runtime_1.db.collection(constants_1.COLLECTIONS.JOBS).doc(jobId).collection('timecards').get();
...
async function listSubmittedTimecardsForJobWeek(jobId, weekStart) {
    ...
    const directSnap = await runtime_1.db
        .collection(constants_1.COLLECTIONS.JOBS)
        .doc(jobId)
    ...
    const legacySnap = await runtime_1.db
        .collection(constants_1.COLLECTIONS.JOBS)
        .doc(jobId)
...
async function resolveCreatorNames(timecards) {
    ...
    const snapshots = await Promise.all(uniqueUids.map(async (uid) => {
        const snap = await runtime_1.db.collection(constants_1.COLLECTIONS.USERS).doc(uid).get();
        return { uid, snap };
    }));
    const names = {};
...
exports.listTimecardStaffingEmployees = (0, https_1.onCall)(async (request) => {
    ...
    try {
        const employeesSnap = await runtime_1.db.collection('employees').where('active', '==', true).get();
...
exports.addEmployeeToTimecardRoster = (0, https_1.onCall)(async (request) => {
    ...
    assertAdminOrAssignedForeman(user, jobId, 'Only admins or assigned foremen can manage timecard staffing');
    try {
        const employeeRef = runtime_1.db.collection('employees').doc(employeeId);
        ...
        const rosterCollection = runtime_1.db.collection(constants_1.COLLECTIONS.JOBS).doc(jobId).collection('roster');
...
exports.saveForemanTimecard = (0, https_1.onCall)(async (request) => {
    ...
    try {
        const timecardRef = runtime_1.db.collection(constants_1.COLLECTIONS.JOBS).doc(jobId).collection('timecards').doc(timecardId);
